...
co_owner = None
bgmi_cooldown = {}
DEFAULT_COOLDOWN = timedelta(seconds=3)  # Cooldown time using timedelta for accurate comparison
...
def send_welcome(message):
    ...
    btn_api_server = telebot.types.KeyboardButton('🚀API SERVER🚀')
    
    # Add buttons to markup
    markup.add(btn_attack, btn_attack_url,btn_info)
    markup.add(btn_api_server)
...
def send_fake_messages(chat_id, number, duration):
    end_time = time.time() + duration
    message_count = 0

    while time.time() < end_time:
        for _ in range(5):  # Send 5 messages at a time
            bot.send_message(chat_id, f"Fake message to {number}")
            message_count += 1
            
        time.sleep(1)  # Wait for 1 second before sending the next batch
        print(f"Sent {message_count} messages to {number}...")  # Optional: for logging purposes

@bot.message_handler(commands=['bomb'])
def bomb_command(message):
    bot.reply_to(message, "Please enter the number (format: +1234567890):")
    bot.register_next_step_handler(message, process_number)

def process_number(message):
    number = message.text
    chat_id = message.chat.id
    user_requests[chat_id] = {'number': number}

    bot.reply_to(message, "Please enter the duration in seconds:")
...
def process_duration(message):
    ...
    try:
        ...
        number = user_requests[chat_id]['number']

        bot.reply_to(message, f"Sending fake messages to {number} for {duration} seconds...")
        
        # Start sending messages in a new thread
        threading.Thread(target=send_fake_messages, args=(chat_id, number, duration)).start()
    
    except ValueError:
        bot.reply_to(message, "Please enter a valid number for duration.")
        return
...
def process_approval(message):
    try:
        ...
        if user_id in authorized_users and isinstance(authorized_users[user_id], dict):
            existing_expiry = authorized_users[user_id].get('expiry', current_time)
            new_expiry = max(current_time, existing_expiry) + duration
        else:
            new_expiry = current_time + duration

        # Update authorized_users with the new expiry date as part of a dictionary
        authorized_users[user_id] = {'expiry': new_expiry}
        save_authorized_users()

        # Log the approval and notify
        user_info = bot.get_chat(user_id)
        username = user_info.username if user_info.username else f"ID: {user_id}"
        bot.send_message(message.chat.id, f"User @{username} (ID: {user_id}) has been approved for {duration_text}.")
    except Exception as e:
...
def admin_dashboard(message):
    if message.from_user.id in {YOUR_OWNER_ID, co_owner}:
        ...
        bot.send_message(
            message.chat.id,
            f"Admin Dashboard:\n\nActive Users: {active_users}\nTotal User Balance: {total_balance}"
        )
    else:
